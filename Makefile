.PHONY: install test lint format api download-ledgar prepare-data train-baseline train-transformer-debug evaluate build-index analyze-sample docker-build docker-run
...
docker-run:
	docker run --rm -p 8000:8000 -e CONTRACT_RISK_MOCK_MODEL=true -v "$$(pwd)/models:/app/models" -v "$$(pwd)/data:/app/data" contract-risk-analyzer:local
